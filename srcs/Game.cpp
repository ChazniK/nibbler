...
/*   Updated: 2018/08/12 11:38:39 by mafernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	void				*dl_handle;
	void				(*fn_pointer)(int width, int height, bool debug);

	const char *library = windowSettings().c_str();
	dl_handle = dlopen( library, RTLD_LAZY | RTLD_LOCAL );
	if ( !dl_handle )
		dlerrorWrapper();
	fn_pointer = (void(*)(int width, int height, bool debug)) dlsym(dl_handle, "Display");
	if (!fn_pointer)
		dlerrorWrapper();
	fn_pointer(_width, _height, true);
}
